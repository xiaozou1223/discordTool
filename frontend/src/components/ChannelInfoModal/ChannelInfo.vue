...
<template>
  <div class="container" style="text-align: center; background: #322e2e; padding-top: 10px;">
    ...
    <div v-for="role of allowRoles" class="row" style="margin-right: 0px; margin-left: 0px; margin-top: 5px" :key="role.id">
      <div class="col-2" style="text-align: center;padding-top: 5px; padding-bottom: 5px">
        <svg width="30" height="30" viewBox="0 0 24 24" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
            <path :fill="`#${role.color.toString(16).padStart(6, '0')}`" fill-rule="evenodd" d="M3.47 5.18c.27-.4.64-.74 1.1-.96l6.09-3.05a3 3 0 0 1 2.68 0l6.1 3.05A2.83 2.83 0 0 1 21 6.75v3.5a14.17 14.17 0 0 1-8.42 12.5c-.37.16-.79.16-1.16 0A14.18 14.18 0 0 1 3 9.77V6.75c0-.57.17-1.11.47-1.57Zm2.95 10.3A12.18 12.18 0 0 0 12 20.82a12.18 12.18 0 0 0 5.58-5.32A9.49 9.49 0 0 0 12.47 14h-.94c-1.88 0-3.63.55-5.11 1.49ZM12 13a4 4 0 1 0 0-8 4 4 0 0 0 0 8Z" clip-rule="evenodd" class=""></path>
        </svg>
      </div>
      <div class="col" style="text-align: center; padding-top: 5px; padding-bottom: 5px">
    ...
    <hr>
    <div v-for="user of allowUsers" class="row" style="margin-right: 0px; margin-left: 0px; margin-top: 2px; margin-block-end: 2px;" :key="user.user.id">
      <div class="col-2" style="text-align: center">
        <img v-if="user.user.avatar" class="rounded-circle" width="50px" height="50px" :src="`https://cdn.discordapp.com/avatars/${user.user.id}/${user.user.avatar}.png`" />
        <img v-else class="rounded-circle" width="50px" height="50px" :src="`https://www.pngkit.com/png/full/816-8165219_discord-logo-png-discord-icon.png`" />
      </div>
      <div class="col" style="text-align: center;">
        <span class="selectable-text" style="font-weight: bold; font-size: 18px; color: white"> {{ user.nick || user.user.username || user.user.global_name }}</span>
      </div>
    </div>
    <div v-for="user of denyUsers" class="row" style="margin-right: 0px; margin-left: 0px; margin-top: 2px; margin-block-end: 2px;" :key="user.user.id">
      <div class="col-2" style="text-align: center">
        <img v-if="user.user.avatar" class="rounded-circle" width="50px" height="50px" :src="`https://cdn.discordapp.com/avatars/${user.user.id}/${user.user.avatar}.png`" />
        <img v-else class="rounded-circle" width="50px" height="50px" :src="`https://www.pngkit.com/png/full/816-8165219_discord-logo-png-discord-icon.png`" />
      </div>
      <div class="col" style="text-align: center;">
        <span class="selectable-text" style="font-weight: bold; font-size: 18px; color: red"> {{ user.nick || user.user.username || user.user.global_name }}</span>
      </div>
    </div>
  </div>
</template>
